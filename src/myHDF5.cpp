...
write_hdf5(T * value, H5::H5File * file, const std::string name) {
  hsize_t    dims[2], chunk_dims[2]; // dataset dimensions
  dims[0] = chunk_dims[0] = 1;
  dims[1] = chunk_dims[1] = 1;
  H5::DataSet dataset;
  ...
  dataset.write(value, complex_datatype);
};
...
write_hdf5(T * value, H5::H5File *  file, const std::string  name) {
  ...
  dataset.write(value, DataTypeFor<T>::value);
};
...
template <typename T> 
void instantiateHDF<T>:: write_hdf5A(T* mu, H5::H5File * file, const std::string name) {
  write_hdf5<T>(mu, file, name);
};
